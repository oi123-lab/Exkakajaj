...
Tab1:AddParagraph({"Olá, " .. playerName .. "!"})
...
local MaxClicksPerCycle = 10  -- Limite de cliques por ciclo
local ClickDelay = 0.00  -- Tempo de delay entre os cliques para otimizar
...
Toggle = Tab1:AddToggle({
    ...
    playClickSound()
        if Value then
            ...
            local LaptopPath = { "WorkspaceCom", "001_GiveTools", "Laptop" }

            -- Função para localizar o Laptop
            local function GetLaptop()
                local current = WorkspaceService
                for _, part in ipairs(LaptopPath) do
                    current = current:FindFirstChild(part)
                    if not current then
                        warn("Parte não encontrada na cadeia:", part)
                        return nil
                    end
                end
                return current
            ...
            local Laptop = GetLaptop()
            if not Laptop then return end

            local ClickDetector = Laptop:FindFirstChildOfClass("ClickDetector")
            if not ClickDetector then
                warn("ClickDetector não encontrado no Laptop.")
                return
            end

            -- Função otimizada para destruir o Laptop do inventário e do personagem
            local function DestroyLaptopItems()
                -- Remove os itens "Laptop" do inventário e do personagem
                local function RemoveLaptop(itemContainer)
                    for _, tool in ipairs(itemContainer:GetChildren()) do
                        if tool.Name == "Laptop" then
                            tool:Destroy()
                        end
                    end
                end
                RemoveLaptop(Player.Backpack)
                RemoveLaptop(Character)
            end

            -- Função para acionar o ClickDetector
            ...
            task.spawn(function()
                while LaptopActive do
                    -- Remover os itens Laptop a cada ciclo
                    DestroyLaptopItems()

                    -- Limite de cliques por ciclo
                    for i = 1, MaxClicksPerCycle do
                        -- Move o personagem para o Laptop
                        if RootPart and Laptop then
                            RootPart.CFrame = Laptop.CFrame

                            -- Aciona o ClickDetector
                    ...
                    -- Pausa entre ciclos para reduzir sobrecarga
                    task.wait()  -- Ajuste o tempo de espera conforme necessário
                end
            end)
        else
            LaptopActive = false
...
local LaptopActive = false
local MaxClicksPerCycle = 10  -- Limite de cliques por ciclo para evitar sobrecarga
local ClickDelay = 0.00  -- Tempo de delay entre os cliques para otimizar a velocidade sem sobrecarregar
...
Toggle = Tab1:AddToggle({
    ...
    playClickSound()
        if Value then
            ...
            local function DestroyLaptopItems()
                -- Remove o Laptop do inventário/localmente para evitar lag
                ...
                for _, tool in ipairs(Character:GetChildren()) do
                    if tool:IsA("Tool") and tool.Name == "Laptop" then
                        tool:Destroy()
                    end
                end
            end
                ...
                while LaptopActive do
                    ...
                    for i = 1, MaxClicksPerCycle do
                        ...
                        if RootPart and Laptop then
                            ...
                            -- Ativa o ClickDetector
                            TriggerClickDetector()

                            -- Remove o Laptop do inventário/localmente
                            DestroyLaptopItems()

                            -- Atraso entre cliques para otimizar
                            task.wait(ClickDelay)
                        end
                    end
                    -- Pausa entre ciclos para reduzir sobrecarga
...
local Section = Tab2:AddSection({"<Combate Ball-"})
...
-- Configura a bola para seguir o alvo e causar fling com BodyPosition próximo
function setupBall()
	local targetName = getgenv().Target
	if not targetName then
	...
	-- Força leve constante pra manter a bola flutuando
	...
	connection = RunService.Heartbeat:Connect(function()
		...
		local offset = forward * (toggle and 2 or -2) -- DISTÂNCIA CURTA
...
-- Botão que ativa o Fling com base no alvo selecionado
Tab2:AddButton({
	Name = "Fling Ball",
	Callback = function()
    playClickSound()
...
local Section = Tab2:AddSection({"<Combate Sofá-"})
...
getgenv().OldPos = nil -- Posição inicial antes do Fling
...
local function SafeReturn()
    ...
    -- Configuração da posição fixa de retorno  
    ...
    -- Move direto para a posição definida  
...
local function SkidFling(TargetPlayer)
    ...
    -- Salva a posição inicial do jogador antes de iniciar o fling  
    ...
    local function SFBasePart(BasePart)  
        ...
        repeat  
            ...
            if THumanoid then  
                ...
                -- Ajustando a altura e rotação no alvo  
                ...
                -- Comportamento alternativo se o humanoide não for encontrado  
        ...
        -- No último segundo, desequipa o Couch e tira as forças  
        UnequipCouch()  

        -- Remove a força de impulso e velocity  
        BV:Destroy()  

        -- Volta à posição original com altura corrigida  
        local ReturnPos = getgenv().OldPos  
        if RootPart.Position.Y > ReturnPos.Position.Y then  
            -- Se o jogador foi lançado para uma posição mais alta, devolve ele para a posição original com altura ajustada  
        ...
        -- Desabilita a física antes de devolver o jogador  
        Humanoid.PlatformStand = true  

        -- Restaura a posição do jogador  
        RootPart.CFrame = ReturnPos  

        -- Habilita a física novamente após a restauração da posição  
        task.wait(1)  
        Humanoid.PlatformStand = false  
        Humanoid:SetStateEnabled(Enum.HumanoidStateType.Seated, true)

        -- Chama a função de retorno seguro após o Fling
        SafeReturn()
    end  
...
local Section = Tab2:AddSection({"<Combate Veículo-"})
...
Tab2:AddButton({
    ...
    playClickSound()
        ...
        -- Função segura para recuperar alvo continuamente
        ...
        -- Loop principal com persistência
        local Angles = 0
        local YRotation = 0
        ...
        while PCar.Parent do
            ...
            -- Verifica se o alvo ainda existe; se não, espera o retorno
...
Tab2:AddButton({
    ...
    Callback = function()
    playClickSound()
        -- Executa o remote para deletar todos os veículos
        local args = {
            [1] = "DeleteAllVehicles"
        }
                ...
                if TargetC and TargetH and TargetRP then
                    if not TargetH.Sit then
                        while not TargetH.Sit do
                            ...
                            -- Aumentando a rotação progressivamente
                            Angles = Angles + 100
                            YRotation = YRotation + 5000  -- Adicionando rotação no eixo Y
            ...
            if TargetC and TargetH and TargetRP then
                if not TargetH.Sit then
                    ...
                    while not TargetH.Sit do
                        ...
                        -- Função para movimentar o carro e alinhar o ataque
    ...
    if TargetC and TargetH and TargetRP then
        if not TargetH.Sit then
            ...
            while not TargetH.Sit do
                ...
                -- Mantendo a rotação alinhada com a lógica
                Angles = Angles + 100
                YRotation = YRotation + 5000
                local Rotation = CFrame.Angles(math.rad(Angles), math.rad(YRotation), 0)
...
-- Botão que ativa o Fling Ball em todos os jogadores
Tab2:AddButton({
	Name = "Fling Ball All",
	Callback = function()
    playClickSound()
...
getgenv().Target = nil -- Alvo global
local forceActive = false -- Controla se a força está ativa
local BV -- Variável para armazenar a BodyVelocity
...
local function UnequipCouch()
    -- Não vamos permitir que o Couch seja desequipado
end
...
local function SkidFling(TargetPlayer)
    ...
    -- Configura a força (BodyVelocity) com "um googolplex de força" (simbolicamente math.huge)
    BV = Instance.new("BodyVelocity")
    BV.Parent = RootPart
    BV.Velocity = Vector3.new(1e12, 1e12, 1e12)
    BV.MaxForce = Vector3.new(math.huge, math.huge, math.huge) -- Máxima força possível no Roblox (equivalente a "infinito")

    Humanoid:SetStateEnabled(Enum.HumanoidStateType.Seated, false)

    SFBasePart(TRootPart)

    -- Deixa a força ativa até o final
    forceActive = true
    ...
    -- Remove a força apenas no final da função
    if BV then
        BV:Destroy()
    end
end
...
local function onDeath()
    -- Interrompe o Fling e a força se o jogador morrer
    forceActive = false
    if BV then
...
Tab2:AddButton({
    ...
    playClickSound()
        ...
        -- Lista atualizada de nomes de usuários na whitelist
        local Whitelist = {
            ["kauxz016"] = true,
            ["goduser573"] = true,
            ["NOEL_A21S"] = true,
                ...
                if Target ~= Player and Target.Character then
                    ...
                    if not Whitelist[Target.Name] then
                        ...
                        if TargetH and TargetRP then
                            ...
                            if not TargetH.Sit then
                                local Angles = 0 -- Inicializa a rotação
                                ...
                                while not TargetH.Sit do
                                    ...
                                    -- Aumentando a rotação progressivamente
                                    Angles = Angles + 100
                                    local Rotation = CFrame.Angles(0, math.rad(Angles), 0) -- Mantém a rotação apenas no eixo Y
...
local Section = Tab3:AddSection({"Aba Teleports"})

-- Configuração do Monitoramento de Erros
local function safeCall(func)
    local success, err = pcall(func)
    if not success then
        warn("Erro detectado: " .. err)
        -- Caso deseje tentar corrigir o erro automaticamente, podemos aplicar uma solução de fallback aqui
    end
end
...
local function teleportWithTween(targetPosition)
    ...
    -- Garantir que a posição do jogador e o humanoidRootPart são válidos
    if targetPosition and humanoidRootPart then
        -- Criar Tween para mover suavemente o jogador até a posição
...
-- Movendo os botões para a Tab3 com Teleporte e Monitoramento de Erros
Tab3:AddButton({
...
local Tab5 = Window:MakeTab({"House", "rbxassetid://10723407389"})

local Section = Tab5:AddSection({"Aba House"})

-- Seção: Funcionalidades de Casa
local Section = Tab5:AddSection({"<Funcionalidades de Casa>"})
...
-- Criação da dropdown com números de 1 a 37
local Dropdown = Tab5:AddDropdown({
    ...
    Callback = function(Value)
    playClickSound()
        selectedValue = tonumber(Value)  -- Converte a seleção para número
    end
})

-- Botão para disparar os remotes com base no número selecionado
Tab5:AddButton({
    Name = "Lock House New",
    Callback = function()
    playClickSound()
        ...
        -- Após 0.02 segundos, envia PlayerSellHouse
...
local Paragraph = Tab5:AddParagraph({"Permissão De Casa", "House Perm"})

-- Seção: Permissões e Teleporte
local Section = Tab5:AddSection({"<Permissões e Teleporte>"})
...
local List_House = Tab5:AddDropdown({
  Name = "Lista de casas",
  Options = {},
  Default = {"..."},
  MultSelect = false,
...
-- Botão para pegar permissão da casa
Tab5:AddButton({
...
-- Botão para teleportar para a casa
Tab5:AddButton({
...
-- Seção: Remoção de Banimento
local Section = Tab5:AddSection({"<Remoção de Banimento>"})
...
-- Seção: Personalização de Casa
local Section = Tab5:AddSection({"<Personalização de Casa>"})
...
-- Seção: Ações Diversas
local Section = Tab5:AddSection({"<Ações Diversas>"})

local runningBaby = false
ToggleBaby = Tab5:AddToggle({
...
local ToggleLockDoors = Tab5:AddToggle({
...
local Tab6 = Window:MakeTab({"Car", "rbxassetid://10709783137"})

local Section = Tab6:AddSection({"Aba Car"})

-- Botão para coletar ovos (35 vezes)
Button = Tab6:AddButton({
    Name = "Coletar Ovos (35x)",
    Callback = function()
    playClickSound()
        local success, err = pcall(function()
            local eggRemote = game:GetService("ReplicatedStorage"):FindFirstChild("Remotes") and game.ReplicatedStorage.Remotes:FindFirstChild("EggCollected")

            if not eggRemote or not eggRemote.InvokeServer then
                warn("[SHNMAX] Remote 'EggCollected' não encontrado ou inválido!")
                return
            end
        ...
        if success then
            print("[SHNMAX] 35 ovos coletados com sucesso!")
        else
            warn("[SHNMAX] Erro ao coletar ovos: " .. tostring(err))
        end
    end
})

local Section = Tab6:AddSection({"Velocidade"})
...
local Section = Tab6:AddSection({"Music Box"})
...
local RainbowCarToggle = Tab6:AddToggle({
...
local Section = Tab7:AddSection({"Aba Avatar"})
...
Tab9:AddSection({"Audio all através de lista de IDS"})

local soundList = {
    {Name = "Grito Alto", ID = 5710016194},       -- Grito Alto
    {Name = "Jumpscare Horroroso", ID = 85435253347146}, -- Jumpscare Horroroso
    {Name = "Áudio Alto", ID = 6855150757},        -- Áudio Alto
    {Name = "Ruído", ID = 120034877160791},        -- Ruído
...
local ReplicatedStorage = game:GetService("ReplicatedStorage")
local audioID = 5229833733 -- ID fixo do áudio
...
Tab9:AddSection({"Audio all através de (você que colocar seus próprios ids)"})
...
-- Criação do Dropdown de seleção de áudio
Tab9:AddDropdown({
    ...
    playClickSound()
        for _, sound in ipairs(soundList) do
            if sound.Name == selectedName then
                -- Quando o áudio for selecionado, copia o ID
                local selectedID = sound.ID
                setclipboard(tostring(selectedID))  -- Copia o ID para a área de transferência
...
-- Seção: Modificações de Jogador
local Section = Tab9:AddSection({"<Modificações de Jogador>"})
...
local Section = Tab9:AddSection({"<Configurações de Ambiente>"})
...
-- Seção: Funcionalidades de Movimentação
local Section = Tab9:AddSection({"<Funcionalidades de Movimentação>"})
...
-- Seção: Ações e Interações
local Section = Tab9:AddSection({"<Ações e Interações>"})
...
-- Seção: Funcionalidades de Jogo
local Section = Tab9:AddSection({"<Funcionalidades de Jogo>"})
...
-- Função para criar um tornado de PaintRollers 
...
-- Adiciona um botão para criar o tornado 
Section = Tab9:AddSection({ Name = "Tornado de PaintRollers" })

Tab9:AddButton({ Name = "Criar Tornado de PaintRollers (Altura: 100, Raio: 5)", Callback = function() 
    playClickSound()
    createPaintRollerTornado(70, 5, 20, 0.5)  -- Ajuste os parâmetros conforme necessário 
end })

Tab9:AddButton({ Name = "Parar Processo de Construção", Callback = function() 
    playClickSound()
    stopProcess = true 
    resetCharacter() 
end })
...
Tab9:AddButton({
	Name = "Mudar Animação do Tornado de PaintRoller",
	Callback = function()
    playClickSound()
        ...
        task.wait(0.2) -- Dá tempo para mover tudo com segurança
...
local Tab10 = Window:MakeTab({"Esp", "rbxassetid://10747382504"})

local Section = Tab10:AddSection({"Aba Esp"})
...
local Section = Tab11:AddSection({"Aba Scripts"})

Tab11:AddButton({
  Name = "Infinite Yield Shnmaxhub",
  Callback = function()
    playClickSound()
    loadstring(game:HttpGet('https://pastebin.com/raw/EB5rLyTz'))()
  end
})
